...
export default {
    ...
    methods: {
        ...
        index(page = 1) {
            axios.get('/business/conferences?page=' + page)
                .then((response) => {
        ...
        updateConference() {
            this.loading = true;
            this.form.patch('/business/conferences/' + this.form.id + '/edit')
                .then(() => {
                    Vue.notify({
                        group: 'foo',
                        type: 'primary',
        ...
        destroy(item) {
            Swal.fire({
                ...
                if (result.value) {
                    axios.delete('/business/conferences/' + item.id)
                        .then(() => {
                            Swal.fire(
                                'Deleted!',
        ...
        createConference() {
            this.loading = true;
            this.form.post('/business/conferences/create')
                .then(() => {
                    Vue.notify({
                        group: 'foo',
                        type: 'primary',
        ...
        addPassword() {
            this.loading = true;
            this.passwordForm.post('/business/conferences/' + this.conferenceId + '/add-password')
                .then(() => {
                    Vue.notify({
                        group: 'foo',
                        type: 'primary',
    ...
    computed: {
        rootUrl: function () { return window.rootUrl; },
        ...
        <div class="col-sm-12">
            <div class="white-box">
                ...
                <div class="table-responsive">
                    <table class="table table-striped mt-4" data-form="deleteForm">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in business.data" :key="item.id">
                                ...
                                <td>
                                    {{ item.name }}<br>
                                    <a :href="rootUrl + '/business/conferences/' + item.slug" target="_blank">Go
                                        Live</a> <i class="fa fa-external-link"></i>
                                </td>
                                <td><i class="fa fa-clock-o"></i> {{ toLocalTime(item.start_time) }}</td>
                                <td>
                                    <button v-clipboard:copy="rootUrl + '/business/conferences/' + item.slug"
                    ...
                    <div class="d-flex justify-item-center" v-if="business.length == 0" colspan="6">
                        ...
                        <div class="modal-body">
                            <div class="form-body">
                                <div class="form-group" :class="{ 'has-error': form.errors.has('name') }">
                                    ...
                                    <has-error :form="form" class="text-danger" field="name"></has-error>
                                </div>
                                <!-- <div class="form-group">
                                    <label for="start_time">Start Time( Default NOW)</label><br>
                                    <date-picker v-model="form.start_time" type="datetime" format="YYYY-MM-DD hh:mm a"
                                ...
                                <div class="form-group">
                                    ...
                                    <has-error :form="form" class="text-danger" field="time_range"></has-error>
                                </div>
                                <div class="form-group" :class="{ 'has-error': form.errors.has('password') }">
                                    ...
                                    <has-error :form="form" class="text-danger" field="password"></has-error>
                                </div>
                            </div>
                        </div>
