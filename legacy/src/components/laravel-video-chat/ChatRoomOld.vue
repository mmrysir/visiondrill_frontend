<template>
  <div>
    <div class="row">
      <div class="col-md-6">
        <div class="panel panel-info">
          <div class="panel-heading">
            ...
            Chat with {{ withUser.fullname }}
            <button
              class="btn btn-warning btn-sm pull-right"
              @click="startVideoCallToUser(withUser.id)"
              type="button"
            >
          ...
          <div class="panel-body">
            <ul class="chat" v-chat-scroll>
              <li
                ...
                v-bind:class="{ 'right' : check(message.sender.id), 'left' : !check(message.sender.id) }"
              >
                <span
                  class="chat-img"
                  v-bind:class="{ 'pull-right' : check(message.sender.id) , 'pull-left' : !check(message.sender.id) }"
                >
                  <img
                  ...
                  <div class="header">
                    ...
                    <small class="text-muted">
                      ...
                      <timeago
                        ...
                        :datetime="new Date(message.created_at + 'Z')"
                        :auto-update="10"
                        locale="zh-CN"
                      ></timeago>
                    </small>
                    <strong
                      v-bind:class="{ 'pull-right' : check(message.sender.id) , 'pull-left' : !check(message.sender.id)}"
                      class="primary-font"
                    >{{ message.sender.fullname }}</strong>
                  </div>
                  <p
                    v-bind:class="{ 'pull-right' : check(message.sender.id) , 'pull-left' : !check(message.sender.id)}"
                  >{{ message.text }}</p>
        ...
        <div class="modal-dialog">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" id="answerCallButton" class="btn btn-success">
                <i class="fa fa-phone"></i> Answer
              </button>
              <button type="button" id="denyCallButton" data-dismiss="modal" class="btn btn-danger">
      ...
      <div id="outgoingVideoCallModal" class="modal fade" role="dialog">
        <div class="modal-dialog">
          <!-- Modal content-->
          <div class="modal-content">
            <div class="modal-header">
      ...
      <audio controls id="player_audio" loop hidden>
        <source :src="this.audio_sound" type="audio/wav" />Your browser does not support the audio element.
      </audio>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      audio_sound:
        "https://soundbible.com/mp3/Phone_Ringing_8x-Mike_Koenig-696238708.mp3"
    };
  },
  methods: {
    cancelCall() {
    ...
    startVideoCallToUser(id) {
      $("#outgoingVideoCallModal").modal("show");
      this.playSound(this.audio_sound);
    ...
    listenForNewMessage: function() {
      Echo.join(this.channel)
        ...
        .listen(
          "\\PhpJunior\\LaravelVideoChat\\Events\\NewConversationMessage",
          data => {
            var self = this;
        ...
        .listen(
          "\\PhpJunior\\LaravelVideoChat\\Events\\VideoChatStart",
          data => {
...
function onSignalAnswer(answer) {
  ...
  $("#outgoingVideoCallModal").modal("hide");
...
function call() {
  ...
  console.log('Here is the test: ' + pc);
...
function onAnswer() {
  ...
  console.log('third log' + remoteOffer);
