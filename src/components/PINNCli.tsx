    ...
    <div className="h-full flex flex-col bg-[#0F0F12] border-l border-white/5">
       <div className="p-4 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
          <div className="flex items-center gap-2">
             ...
             <span className="text-[10px] font-mono text-white font-black uppercase tracking-widest leading-none">PINN-AI CLI</span>
          </div>
       ...
       {/* CLI Log Output */}
       <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] space-y-3 custom-scrollbar">
          {logs.map((log, i) => (
            <div key={i} className="flex gap-3 animate-in slide-in-from-left-2 duration-300">
              ...
              <div className="flex-1 min-w-0">
                <span className={`uppercase font-black mr-2 ${
       ...
       <div className="p-4 bg-black/20 border-t border-white/5">
          <form onSubmit={handleCommand} className="relative">
             <div className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-500 font-mono text-[11px] font-black select-none">{'>'}</div>
             <input 
               type="text"
               value={input}
               onChange={(e) => setInput(e.target.value)}
               placeholder="Type /help for command list..."
               className="w-full bg-white/5 border border-white/5 rounded-lg pl-10 pr-12 py-3 text-[11px] font-mono text-white outline-none focus:border-blue-500/50 transition-all placeholder-gray-700"
               autoFocus
             />
             <button 
               type="submit" 
